...
const UPLOAD_DIR = path.join(ensureUploadDirs(), 'ielts', 'speaking');

const storage = multer.diskStorage({
  destination: (_req, _file, cb) => cb(null, UPLOAD_DIR),
  filename: (_req, file, cb) => {
    const ext = path.extname(file.originalname) || '.webm';
    cb(null, `speak-${Date.now()}-${Math.round(Math.random() * 1e9)}${ext}`);
  },
});
...
module.exports = { uploadSpeaking, UPLOAD_DIR };
